...
//                  and Close according to the 3rd input Flag.
...
//                  NOTE: Toggling the visibility of a figure changes the HWnd!
...
//
//   Enabling the Alpha blending let the figure flashes sometimes. It might be
...
//   Window") helped usually. But I suggest the Painters or ZBuffer renderer
...
//   mattes the Matlab colors [88/255,0,0] to [95/255,0,0]. To be sure,
...
/*
% $JRev: R-~ V:098 Sum:VRSezxgACMVL Date:02-Aug-2021 19:41:35 $
...
% 097: 11-Jan-2021 21:44, In help: "Right, Bottom" replaced by "Width, Height".
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  // Check inputs:
  if (nrhs < 1 || nrhs > 4) {
     if (nrhs == 0) {  // A short hello:
       mexPrintf("Source:   " __FILE__ "\n"
     ...
     if (STRCMP_NI(Command, "topmost", 7) == 0) {
        ...
        // Locking the cursor can concern one window only, therefore no handle
        // is required.
        ClipCursor(NULL);
...
HWND myGetHWnd(const FIG_T Fig_Handle)
{
  ...
  Tmp_NameC = (char *) mxCalloc(Name_Len + 10, sizeof(char));
  if (Tmp_NameC == NULL) {
...
mxArray *myMonitorInfo(HWND hWnd)
{
  ...
  // FullPosition equals WorkPosition, if a monitor does not have a task bar.
